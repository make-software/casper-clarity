...
describe('DeployArgumentParser', () => {
  it('validate and parse bool', () => {
    ...
    expect(DeployArgumentParser.validateBoolean(1)).to.include("not a valid boolean literal");
    expect(DeployArgumentParser.validateBoolean({"a": "a"})).to.include("not a valid boolean literal");
  });
});
